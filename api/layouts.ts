...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  try {
    ...
    if (req.url?.match(/^\/api\/layouts\/\d+$/) && req.method === 'GET') {
      const layoutId = req.url.split('/').pop();
      return new Promise((resolve) => {
    ...
    if (req.url?.match(/^\/api\/layouts\/\d+$/) && req.method === 'PUT') {
      const layoutId = req.url.split('/').pop();
      const { title, patchbays, connections } = req.body;
    ...
    if (req.url?.match(/^\/api\/layouts\/\d+$/) && req.method === 'DELETE') {
      const layoutId = req.url.split('/').pop();
      return new Promise((resolve) => {
